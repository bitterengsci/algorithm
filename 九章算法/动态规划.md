...
- [1. 动态规划 Dynamic Programming](#1-%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92-dynamic-programming)
...
- [6. 区间类动态规划 Interval DP](#6-%E5%8C%BA%E9%97%B4%E7%B1%BB%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92-interval-dp)
- [7. 双序列型动态规划](#7-%E5%8F%8C%E5%BA%8F%E5%88%97%E5%9E%8B%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92)
- [8. 背包型动态规划 backpack](#8-%E8%83%8C%E5%8C%85%E5%9E%8B%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92-backpack)
    - [8.1. 0-1背包 knapsack](#81-0-1%E8%83%8C%E5%8C%85-knapsack)
    - [8.2. 完全/无穷背包](#82-%E5%AE%8C%E5%85%A8%E6%97%A0%E7%A9%B7%E8%83%8C%E5%8C%85)
    - [8.3. 多重背包](#83-%E5%A4%9A%E9%87%8D%E8%83%8C%E5%8C%85)
    - [8.4. 优化拓展 空间优化](#84-%E4%BC%98%E5%8C%96%E6%8B%93%E5%B1%95-%E7%A9%BA%E9%97%B4%E4%BC%98%E5%8C%96)
- [9. DP面试要求 & 现状:](#9-dp%E9%9D%A2%E8%AF%95%E8%A6%81%E6%B1%82--%E7%8E%B0%E7%8A%B6)
- [10. 重点/经典题型](#10-%E9%87%8D%E7%82%B9%E7%BB%8F%E5%85%B8%E9%A2%98%E5%9E%8B)
- [11. 第五讲 （旧）](#11-%E7%AC%AC%E4%BA%94%E8%AE%B2-%E6%97%A7)
    - [11.1. 博弈类DP 游戏, 2人博弈 90%都是DP](#111-%E5%8D%9A%E5%BC%88%E7%B1%BBdp-%E6%B8%B8%E6%88%8F-2%E4%BA%BA%E5%8D%9A%E5%BC%88-90%25%E9%83%BD%E6%98%AFdp)
- [12. 第六讲 （旧）](#12-%E7%AC%AC%E5%85%AD%E8%AE%B2-%E6%97%A7)
    - [12.1. 区间类DP](#121-%E5%8C%BA%E9%97%B4%E7%B1%BBdp)
    - [12.2. 背包类Dp](#122-%E8%83%8C%E5%8C%85%E7%B1%BBdp)

<!-- /TOC -->

This note includes:
- 九章算法班v4.2第九章 动态规划  (Done)
- 九章算法提高班(2019) Chapter 5 & 6  (Done)
- 九章算法背包四讲  (TODO)
- 九章算法强化班(旧) Chapter 5 & 6 (TODO)
- 九章算法动态规划班 Chapter 1~7 (TODO)
...
dynamic programming是一类常用来解决优化问题的算法。它的最大特点是使用子问题的信息帮助解决父问题，使解题的难度减小。
...
dynamic programming这个英文名词有些让人难懂。实际上，这里的programming不是指编程，而是数学上的一种解决优化问题的方法，叫做列表法(tabular method)，大致过程是将函数的不同变量值在表中列出并对表进行各种操作来求得结果。如果列表法是静态（static）的，则动态规划算法中，表格是慢慢增长的，先解决相对简单的子问题，然后通过子问题的结合求取父问题，这样表格好像是“动态”的

动态规划通常有两种解法：top-down和bottom-up
- top-down (Memoization)通常以递归形式出现，从父问题开始，递归地求解子问题。top-down的求解过程通常与memoization结合，即将计算过的结果缓存在数组或者哈希表等结构中。当进入递归求解问题时，先查看缓存中是否已有结果，如果有则直接返回缓存的结果。
- bottom-up (Tabulation)通常以循环形式出现。bottom-up的求解过程通常与tabulation结合，即先解最小的子问题，解决后将结果记录在表格中（通常是一维或二维数组），解决父问题时直接查表拿到子问题的结果，然后将父问题的结果也填在表中，直到把表格填满，最后填入的就是起始问题的结果。

Optimal substructure”，第二个特性叫做“Overlapping sub-problems??? 
...
* **记忆化搜索 = 动态规划的实质** Traditional Dynamic Programming
    ...
    - 自底向上 vs 自顶向下(推荐)
        - Top-Down (with memoization), 自顶向下的将一个问题分解成子问题并逐个求解，通过记录子问题的解来进行优化 (递归树的根节点向下递归调用)
            在传统递归的基础上加入memoization（注意与memorization的区别。memoization来自memo，有备忘的意思），即用数组或表等结构缓存计算结果。在每次递归运算时，先判断想要的结果是否在缓存中，如果没有才进行运算并存入缓存
        - Bottom-Up (with tabulation), 根据状态转移方程从最小的子问题开始一步一步推出最终结果的方式 (从树的叶结点开始不停地向上循环)
            使用循环而非递归，先解决子问题，再利用子问题的答案解决父问题。tabulation即用一个表格存放子问题的答案，然后查表获得父问题需要的所有信息去解决父问题，解决后也填在表中，直至把表填满。
...
最后一步考虑两个序列最后的元素 (2个序列的尾巴, 相连.. 匹配..)
二维数组 f[i][j] 第一个序列前i个和第二个序列前j个(的性质), 可以使用滚动数组优化空间
...
* 完全/无穷背包: n种物品, 每种物品都有无限个。第i个物品的体积为A[i], 价值为V[i]。给一个容量为m的背包。问可以装入背包的最大价值是多少?
    枚举每件物品取0, 1, 2, 3 ... m / A[i] 件     
    dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - k * A[i]] + k * V[i]) 
* 多重背包: 给n种物品, 每种物品都有amounts[i]个 (一定个数)。第i个物品的体积为A[i], 价值为V[i]。给一个容量为m的背包。问可以装入背包的最大价值是多少? 
    枚举每件物品取0, 1, 2, 3 ... amounts[i]件  
    dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - k * A[i]] + k * V[i])

LintCode 92.[Backpack](https://github.com/bitterengsci/algorithm/blob/master/九章算法/动态规划/Backpack.py)
给定N个物品, 重量分别为正整数A0, A1, ..., AN-1, 一个背包最大承重是正整数M, 最多能带走多重的物品
输入: 4个物品, 重量为2, 3, 5, 7 背包最大承重是11    输出:10 (三个物品:2, 3, 5)

每个装物品的方案的总重量都是0到M, 如果对于每个总重量, 我们能知道有没有方案能做到, 就可以解决 (不是单纯的求最大)
背包问题中, 数组大小和总承重有关
- 最后一步: 最后一个物品(重量A[N-1])是否进入背包
- 状态: 设f[i][w] = 能否用前i个物品拼出重量w (True/False)       (求有条件的最大 -> 不超过w的最大)
    常见误区: 错误 --> 设f[i]表示前i个物品能拼出的最大重量(不超过M)   **背包状态中一定要有w代表能拼出的最大重量**
    反例: A=[3, 9, 5, 2], M=10, 前三个物品最大重量为9, 无法取最后一个物品, 故答案为9, 但正确答案为10
    错误原因: 最优策略中, 前N-1个物品拼出的不一定是不超过M的最大重量
- 转移方程: f[i][w] = f[i-1][w] OR f[i-1][w-A[i-1]]
- 初始条件和边界情况:
    f[0][0] = TRUE 0个物品可以拼出重量0
    f[0][1..M] = FALSE 0个物品不能拼出大于0的重量  
    f[i-1][w-A[i-1]]只能在w≥Ai-1时使用
- 计算顺序: 从上到下, 从左到右  f[0][0..m]   ...     f[n][0..m]
答案是最大的j使得f[n][j]=True
TC=O(MN), SC=O(MN), 可以用滚动数组优化空间至O(N) (f只和i, i-1行有关)

BackPack马甲题型
把一个[1, 24, 5, 6]数组尽量平分=和尽量接近
... M/2-1, M/2, M/2+1 ...
        <-  ↓   -> 寻找方向
        先看是否能拼出M/2; 若是不能, 以M/2为中心向两边寻找

LintCode 125.[Backpack II](https://github.com/bitterengsci/algorithm/blob/master/九章算法/动态规划/Backpack%20II.py) 重量+价值 *有价值的背包题目才有价值!*
给定N个物品, 重量分别为正整数A[0], A[1], .. A[N-1], 价值分别为正整数V[0], V[1], ..V[N-1], 一个背包最大承重是正整数M, 最多能带走多大价值的物品
输入: 4个物品, 重量为2, 3, 5, 7, 价值为1, 5, 2, 4, 背包最大承重是11     输出: 9 (物品一+物品三, 重量3+7=10, 价值5+4=9)

每个装物品的方案的总重量都是0到M, 如果对于每个总重量, 我们能知道对应的最大价值是多少, 就能知道答案
**背包问题中, 数组大小和总承重有关**, 背包问题重量一般为正整数(因为重量是一个维度), 但价值可能为负数, 小数..
- 最后一步: 最后一个物品(重量AN-1, 价值VN-1)是否进入背包
- 状态: 设f[i][w] = 用*前i个物品*拼出重量w时最大总价值 (-1表示不能拼出w)
        g[i][w] = 用前i个物品能不能拼出重量w (由于本题价值都为正, 可以用-1表示不能拼出对应价值; 所以g[i][w]并不必要)
- 转移方程: f[i][w] = max{f[i-1][w], f[i-1][w-A[i-1]] + V[i-1] | w≥Ai-1 且 f[i-1][w-A[i-1]]≠-1}
- 初始条件和边界情况:
    f[0][0] = 0     0个物品可以拼出重量0, 最大总价值是0
    f[0][1..M] = -1     0个物品不能拼出大于0的重量
    f[i-1][w-Ai-1]只能在w≥Ai-1, 并且f[i-1][w-Ai-1] ≠-1时使用
- 计算顺序: f[0][0..m]   ... f[n][0..m]
答案是max{f[N][j] | f[N][j]≠-1} where 0<=j<=M
不关心谁的重量大, 只关心谁的钱多 (e.g. M=10kg, 8kg $100, 9kg $1; 显然我们应该取8kg的)

LintCode 440.[Backpack III](https://github.com/bitterengsci/algorithm/blob/master/九章算法/动态规划/Backpack%20III.py) 无穷/完全背包 N个物品 -> N种物品
给定N种物品, 重量分别为正整数A[0], A[1], ..A[N-1], 价值分别为正整数V[0], V[1], ..V[N-1], 每种物品都有无穷多个, 一个背包最大承重是正整数M, 最多能带走多大价值的物品
输入: 4个物品, 重量为2, 3, 5, 7, 价值为1, 5, 2, 4. 背包最大承重是10     输出: 15 (3个物品一, 重量3*3=9, 价值5*3=15)
和Backpack II唯一的不同是: 每种物品都有无穷多个

- 误区: 贪心法错误, 取单位价值最大/性价比最高  (M=10kg, 3kg $3 1$/kg, 10kg $9.1 0.91$/kg, 但取②的价值更大 -> 巧妙运用了剩余空间)

- 状态: 设f[i][w] = 用前i种物品拼出重量w时最大总价值 (-1表示不能拼出w) 
- 转移方程: f[i][w] = max{f[i-1][w-kA[i-1]] + kV[i-1]} where k>=0
        条件 w-kA[i-1]≥0, w/A[i-1]≥k, w最大为M, A[i-1]最小为1, 所以k不会超过M, TC=O(n * m^2)
- 可以转化为 f[i][w] = max{f[i-1][w], f[i][w-A[i-1]] + V[i-1]}   (不需要枚举k, 减少重复计算)
    ```text
           a   b    c    w      w+A[i-1]
    i-1行  △   △    △    △         △           (设A[i-1]=2)   f[i][w] = max{a, b, c}
            ↘︎   ↘︎    ↘︎   ↓         ↓           f[i][w+A[i-1]] = max{f[i-1][w+A[i-1]], a + V, b + V, c + V}
    i行                f[i][w]  f[i][w+A[i-1]]                 = max{f[i-1][w+A[i-1]], f[i][w] + V}  
    ```
- 空间优化
    - 滚动数组: 开2行
    - 实际编程中可以进一步优化: f[i][w] = max{f[i-1][w], f[i][w-Ai-1] + Vi-1} (计算只需要前一行正上方和这一行正左方的值)
        一个一维数组 (从左往右算)
    - 有时从左往右算, 有时从右往左算(正上方和左上方, backpack ii), 有时从右往左或者从左往右都不对(计算一个格子需要前一行正上方, 左上方, 右上方时)  

LintCode 89.[k Sum](https://github.com/bitterengsci/algorithm/blob/master/九章算法/动态规划/k%20Sum.py) 计数
给定数组A, 包含n个互不相等的正整数, 问有多少种方式从中找出K个数, 使得它们的和是Target
输入: A=[1, 2, 3, 4], K=2, Target = 5 输出:2 (1+4=5, 2+3=5)

- 最后一步: 最后一个数A[n-1]是否选入这K个数
- 状态: f[i][k][s]表示有多少种方法可以在前i个数中选出k个, 使得它们的和是s
- 转移方程: A[i-1]不选 + A[i-1]选, 前k-1个数选出和为s-A[i-1]
    f[i][k][s] = f[i-1][k][s] + f[i-1][k-1][s-A[i-1]]|k>=1 AND s>=A[i-1]
- 初始条件:
    f[0][0][0] = 1
    f[0][0][s] = 0, s = 1, 2, ..., Target
- 计算顺序: f[0][0~K][0~Target]    ...   f[N][0~K][0~Target]
答案是f[N][K][Target]
TC=O(N*K*Target) 三维, SC=O(N*K*Target), 可以用滚动数组优化至O(K*Target)
...
## 8.2. 完全/无穷背包
完全背包: n种物品, 每种物品都有无限个。第i个物品的体积为A[i], 价值为V[i]。给一个容量为m的背包。问可以装入背包的最大价值是多少?
...
Highlight:
- Lintcode 602.[Russian Doll Envelopes] 没懂bisect那个。。
- Burst Ballons
- K Edit Distance 字典树
- Backpack III
- K sum 三维动态规划
